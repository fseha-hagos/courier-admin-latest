...
import { MapPin, X, ZoomIn, Loader2, Check, ArrowRight } from "lucide-react"
...
export function CreatePackageComponent() {
  ...
  const customers = useUsersStore((state) => state.users)
  ...
  useEffect(() => {
    fetchUsers();
    console.log("Fetching users", customers)
  }, []);
  ...
  const packageFormSchema = z.object({
    ...
    weight: z.string(),
    pickupLocationId: z.string().nonempty(),
    deliveryLocationId: z.string().nonempty(),
    labels: z.array(z.string()).optional(),
  })
  const packageCreateForm = useForm<z.infer<typeof packageFormSchema>>({
  ...
  function handleFormSubmit(data: z.infer<typeof packageFormSchema>) {
    toast({
  ...
  return (
    <div className="container mx-auto p-4 grid md:grid-cols-2 gap-4 h-[calc(100vh-4rem)]">
      <Form {...packageCreateForm}>

        <div className="flex flex-col space-y-4 overflow-y-auto custom-scrollbar">
          <form onSubmit={packageCreateForm.handleSubmit(handleFormSubmit)}>
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col gap-4 min-h-[13rem]">
                      <FormControl className="w-full">
                        ...
                        <div className={cn(
                          "flex flex-row items-center justify-center gap-4 border-l-input border-[0.5px] rounded-md p-1",
                          !field.value && "text-muted-foreground"
                        )}>
                          {field.value ? <IconUserCheck className="h-4 w-4" /> : <IconUserExclamation className="h-4 w-4" />}
                          {field.value ? customers?.find((customer) => customer.id === field.value)?.name : "No customer selected"}
                        </div>
                      </FormControl>

                      <Command
                        filter={(value, search) => {
                          // Custom filter to search by phone number
                      ...
                      >
                        <CommandInput placeholder="Search by name or phone number..." onValueChange={(value) => setSearchTerm(value)} />
                        {loading ?
                          ...
                          :
                          <CommandList className="overflow-y-auto max-h-[16rem]">
                            ...
                            <CommandGroup>
                              {filteredCustomers?.map((customer) => (
                                <CommandItem
                                  key={customer.id} // Ensure unique key
                                  value={customer.phoneNumber ?? ""} // Searchable by phone number
                                  onSelect={() => {
              ...
              <CardHeader>
                <CardTitle className="flex justify-between items-center">
                  ...
                  <div className="space-x-2">
                    <Button variant="outline" size="sm" aria-label="Clear all locations">
                      Clear Locations
                    </Button>
                  </div>
                </CardTitle>
              </CardHeader>
              <CardContent className="flex flex-col md:grid-cols-2 gap-2">
                ...
                {/* Route Details */}
                {directions && directions.routes[0].legs[0] && !isCalculatingRoute ? (
            ...
            {pickupLocation && pickupLocation !== null && (
              <>
                <Marker
                  ...
                  label="P"
                  draggable={true}
                  // onDrag={(e) => handleMarkerDrag(e.latLng!, "pickup")}
        ...
        {/* Continue Button */}
        <div className="flex items-cente0r justify-center gap-4 col-span-2">
          <Button variant="outline" className="flex-grow items-center justify-center" onClick={clearAllLocations}>
            <IconArrowBack className="w-4 h-4 mr-2" />
            Cancel and go back
          </Button>
          <Button variant="default" className="flex-grow items-center justify-center" type="submit">
            Save and continue
